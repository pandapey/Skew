...
// Part 7 — Employee task submission.
//
// The comment is MANDATORY (the server returns 422 on a blank one).
//
// Attachments: ProjectTask has no per-task file field — files are modelled at
// PROJECT level (the ProjectFile collection). Rather than invent a new upload
// pipeline, a submitted attachment is stored as a real ProjectFile on the
// task's project and linked from the submission record. The field therefore
// accepts a URL to an already-hosted file, matching how ProjectFile is
// populated everywhere else in this codebase (it stores a `url`, not binary).
// This limitation is documented in the Phase 4 report.
