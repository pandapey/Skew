...
export async function runLeaveMaintenance() {
  try {
    ...
    if (expired || reminded || meetingReminders) {
      console.log(`[leave-scheduler] expired=${expired} reminders=${reminded} meetingReminders=${meetingReminders}`)
    }
    return { expired, reminded, meetingReminders }
  } catch (err) {
    console.error('[leave-scheduler] cycle failed:', err?.message)
...
export function startLeaveScheduler() {
  ...
  console.log(`[leave-scheduler] started (every ${intervalMs() / 60000} min)`)
  return timer
}
