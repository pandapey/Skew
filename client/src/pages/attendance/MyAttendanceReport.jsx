...
import { FiArrowLeft, FiClock, FiZap, FiLogIn, FiUserX, FiCalendar } from 'react-icons/fi'
...
// Employee-facing Attendance Report (Issue 3). Redesigned to mirror the Calendar
// concept: a single tabbed view (Daily / Weekly / Monthly / Yearly) where only
// ONE period report is visible at a time, each with its own Date / Month / Year
// / custom-range filter and its own PDF / CSV / Excel download.
//
// ALL data comes from attendanceApi.myHistory ('/attendance/me'), which the
// backend scopes to the caller's own records only — no other employees, no
// Department filter, no RBAC change. Periods are derived client-side from the
// same fetched set; exports reuse the shared ExportMenu + utils/export helpers.
...
const EXPORT_COLUMNS = [
  ...
  { header: 'Overtime', accessor: (r) => r.overtimeHours ?? 0 },
...
// Resolve the inclusive [from,to] (YYYY-MM-DD) window for the active tab.
...
export default function MyAttendanceReport() {
  ...
  const { rows, summary, label } = useMemo(() => {
    ...
    const s = {
      ...
      overtime: +worked.reduce((s2, r) => s2 + (r.overtimeHours || 0), 0).toFixed(1),
    }
    const lbl =
  ...
  return (
    <div>
      ...
      {/* One report at a time, selected by tab (mirrors the Calendar concept). */}
      ...
      <Card className="mb-4">
        ...
        {/* Per-tab filter bar (no Department — employee sees only their own data). */}
        ...
        {/* Period summary — computed from the filtered records. */}
        <div className="mb-4 grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-5">
          ...
          <StatCard label="Overtime" value={`${summary.overtime}h`} icon={FiZap} tone="warning" />
        </div>
