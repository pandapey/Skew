...
// PHASE SALARY/PROJECT AUDIT (WIDGET NAV): read an optional starting day from
// the URL (`/calendar?date=YYYY-MM-DD`). This is the deep-link contract the
// dashboard meeting widgets navigate through, so "open this meeting" lands on
// the day the meeting is actually on instead of on today. An absent, malformed
// or unparseable value falls back to today — the exact previous behaviour — so
// every existing entry point into /calendar is unchanged.
...
export default function CalendarApp() {
  ...
  // PHASE: EMPLOYEE CALENDAR (REQUIREMENT 5) — the calendar is scoped to the
  // currently viewed window at the DATA level. The old query fetched the whole
  // collection (calendarApi.list, capped at 200 masters, no date filter); the
  // month grid then "filtered" by rendering cells, which left the server
  // returning (and the Agenda view showing) events from other months and let
  // the 200-master cap silently drop earlier events. The events query is now a
  // RANGE query over the exact same [windowStart, windowEnd] the recurrence
  // engine expands against (server calendarController.range applies an overlap
  // filter), keyed by the window so navigating months fetches exactly that
  // month — August shows August birthdays, meetings and deadlines.
  ...
  // `windowOcc` is the raw period-scoped stream used to derive which event
  // types are AVAILABLE in the current period (for the sidebar "My Calendars"
  // card) and `occurrences` is what the views actually render.
  const windowOcc = useMemo(() => {
    const expanded = expandEvents(allEvents, windowStart, windowEnd)
    return expanded.filter(
      (o) =>
        // Data-level window guard for the merged (non-recurring) sources —
        // holidays, leaves and project events pass through generateOccurrences
        // unfiltered, so without this check the Agenda view would list events
        // from outside the month. Occurrences inside the window always pass.
