...
export const calendarController = {
  ...
  rescheduleMeeting: async (req, res) => {
    ...
    // PHASE SALARY/PROJECT AUDIT (MEETING BUG 1): this hand-rolled
    // `startAt.getDay() === 0` enforced the Sunday rule but NOT the Company
    // Holiday rule, even though the comment claimed a rescheduled meeting
    // "cannot land somewhere a new one could not be booked" — a company holiday
    // is exactly such a place, and both create paths (staff and client) reject
    // it via meetingDateRejection(). Staff could therefore reschedule a meeting
    // onto Diwali, which no one could have booked directly. It also parsed the
    // day out of a Date object rather than the submitted string, so a UTC-suffixed
    // start could be judged against the wrong calendar day. Reusing the shared
    // rule fixes both and leaves exactly ONE definition of "is this slot bookable".
