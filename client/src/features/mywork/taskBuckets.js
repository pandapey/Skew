...
// Pure grouping logic over the raw task list from `useMyTasks()` — shared by
// the Dashboard's "Today's Tasks" widget and My Work's full bucket view so
// both bucket identically from one fetched result, never duplicating the
// fetch or the rules.
//
// Note: the ProjectTask schema has no dedicated "blocked" status (only
// Todo/In Progress/Review/Done) — "Blocked" is inferred from a `Blocked`
// label or a Bug's `Blocker` severity. This is a client-side heuristic, not a
// real workflow state; a future phase could add a proper `blocked` flag.
