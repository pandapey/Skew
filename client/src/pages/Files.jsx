...
export default function Files() {
  ...
  // PHASE: EMPLOYEE FILES — multi-select works in BOTH views (Files: move to
  // Bin; Recycle Bin: permanent delete). Only file rows are selectable (not
  // folders); selection is scoped to the current view and cleared on
  // navigation / after every bulk operation.
  ...
  // action MOVES files to the Recycle Bin (soft delete), never permanently.
  // The server checks ownership per file; per-file failures come back in
  // `res.failed` and are summarised in the toast.
  ...
  // PHASE: EMPLOYEE FILES (BIN REQUIREMENT) — "Delete Permanently" from the
  // Recycle Bin: removes each file's records AND disk bytes, ownership-checked
  // per file, with per-file failures summarised in the toast.
  ...
  // PHASE: EMPLOYEE FILES (SELECT-ALL FIX) — the listing shows every file in
  // the shared workspace, but the server only lets a user move/delete files
  // THEY own (Admin/Manager may do anything). Select-all used to include
  // foreign files, so "Move all to Bin" failed with "Could not move N files".
  // Selection is now gated to files the user is allowed to act on; foreign
  // files show a disabled checkbox.
  ...
  return (
    <div>
      ...
      {/* Bulk selection toolbar (PHASE: EMPLOYEE FILES) — appears once files are
          selected; Select all / Clear / Delete. In the active view Delete MOVES
          files to the Recycle Bin; in the Bin it permanently deletes. */}
      {files.length > 0 && (
