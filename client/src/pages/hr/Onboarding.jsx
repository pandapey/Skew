...
// New-hire onboarding checklists. Task toggles are local/optimistic until saved.
export default function Onboarding() {
  ...
  const toggleTask = (empId, taskIdx) => {
    setLocal(list.map((o) => {
      if (o.id !== empId) return o
      const tasks = o.tasks.map((t, i) => (i === taskIdx ? { ...t, done: !t.done } : t))
      const done = tasks.filter((t) => t.done).length
      return { ...o, tasks, progress: Math.round((done / tasks.length) * 100) }
    }))
