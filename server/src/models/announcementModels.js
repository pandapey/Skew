...
const postSchema = new mongoose.Schema(
  {
    ...
    // PHASE: EMPLOYEE ANNOUNCEMENT READ STATE — user _ids (as strings) who have
    // read this post. Powers the "unread" badge on the announcements nav item;
    // `read` is DERIVED per request from this array (same pattern as `liked`),
    // and `readBy` is stripped from responses so the feed never leaks readers.
