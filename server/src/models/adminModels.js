...
// --- Activity session ---
// One document per real authentication session. Written by authController on
// successful login (active=true), closed by the logout endpoint (logoutAt set,
// active=false). `user` is the email (kept for legacy seed rows); `userId` is
// the authoritative link to the User document, which is what the admin
// login-history endpoint queries - so history can never bleed across accounts
// that share a name or email.
const activitySchema = new Schema({
  userId: { type: Schema.Types.ObjectId, ref: 'User' },
  user: { type: String, required: true },
  role: { type: String, default: 'Employee' },
  // Client-reported values (derived from navigator.userAgent at login), not
