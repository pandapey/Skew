...
const users = [
  ...
  { name: 'Priya Sharma', email: 'hr@skew.com', password: 'hr123', role: 'HR', department: 'Human Resources', designation: 'HR Manager', gender: 'Female' },
  ...
  { name: 'Sneha Patel', email: 'sales@skew.com', password: 'sales123', role: 'HR', department: 'Sales', designation: 'Sales Executive', gender: 'Female' },
  { name: 'Karan Singh', email: 'finance@skew.com', password: 'finance123', role: 'HR', department: 'Finance', designation: 'Accountant', gender: 'Male' },
...
async function seed() {
  ...
  for (let i = 0; i < 20; i++) {
    ...
    empDocs.push(await Employee.create({
      ...
      workLocation: pick(['Bengaluru HQ', 'Mumbai', 'Delhi', 'Pune', 'Hyderabad'], i),
  ...
  await Payroll.insertMany(empDocs.slice(0, 10).map((e, i) => {
    ...
    const basic = Math.round(monthly * 0.5), hra = Math.round(monthly * 0.2), allowances = Math.round(monthly * 0.3)
    const pf = Math.round(basic * 0.12), tax = Math.round(monthly * 0.08)
    return {
      employee: e.name, empCode: e.empCode, department: e.department, designation: e.designation,
      month: 'July 2026', basic, hra, allowances, pf, tax,
      gross: basic + hra + allowances, net: basic + hra + allowances - pf - tax,
  ...
  for (let i = 0; i < clientCompanies.length; i++) {
    ...
    clientDocs.push(await Client.create({
      ...
      plan: pick(['Business', 'Enterprise', 'Pro'], i), status: i % 6 === 0 ? 'Onboarding' : 'Active',
  ...
  for (let d = 0; d < 10; d++) {
    ...
    empDocs.forEach((e, ei) => {
      ...
      attendanceRecords.push({
        ...
        workingHours, overtimeHours: workingHours > 9 ? +(workingHours - 9).toFixed(1) : 0,
        late, earlyExit: early, status,
      })
    })
  }
