// File-preview helpers. Each parser runs client-side so previews work without a
// server round-trip once the binary bytes are fetched. Heavy libs (SheetJS,
// mammoth) are dynamically imported so they only load when actually previewing
// those file types.
import { fileService } from '@/api/services'

// Pull the raw bytes for a file. fileService returns a Blob fetched from the
// MongoDB (Multer) upload; if there is no binary we get null.
