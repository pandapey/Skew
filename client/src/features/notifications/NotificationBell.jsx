...
// Bell button + live dropdown preview used in the navbar.
export function NotificationBell() {
  ...
  // Phase 6.9 (TASK 8) ROOT CAUSE: this dropdown previewed the FULL list
  // (`list.slice(0, 6)`), read items included. `markAllRead` only flips
  // `read: true` - it never removes anything - so after "Mark all" the six rows
  // were still rendered, merely restyled to the muted/non-bold variant. The
  // badge did clear (unreadCount counts `!n.read`), which is exactly why the
  // two disagreed: the count was unread-based but the list was not.
  //
  // FIX: the dropdown is an UNREAD preview, so it filters on `!n.read`. Once
  // everything is read the list is empty and the existing empty-state below
  // renders, and it repopulates by itself the moment a new notification
  // arrives (local push or the 15s poll). No notification is lost - the full
  // read + unread history still lives on the /notifications page reachable
  // from "View all notifications" at the bottom of this dropdown.
