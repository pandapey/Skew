...
export default function ClientBilling() {
  const { user } = useAuth()
  const { data: billing, isLoading } = useQuery({ queryKey: ['client-payments'], queryFn: () => clientService.getPayments(user) })
  ...
  return (
    <div>
      ...
      <div className="mb-4 grid grid-cols-2 gap-4 lg:grid-cols-4">
        <StatCard label="Total Amount" value={totalAmount} format={formatCurrency} icon={FiFileText} tone="primary" />
              ...
              {payments.map((p) => (
                <div key={p.id} className="flex flex-wrap items-center gap-3 rounded-xl border border-app p-3">
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-xs text-muted">₹{(p.amount || 0).toLocaleString()} · {fmtDate(p.date)}{p.method ? ` · ${p.method}` : ''}</p>
                  </div>
              ...
              {payments.filter((p) => p.status === 'Paid').map((p) => (
                <div key={p.id} className="flex items-center justify-between rounded-xl border border-app p-3">
                  <div>
                    ...
                    <p className="text-xs text-muted">₹{(p.paid || 0).toLocaleString()}</p>
                  </div>
