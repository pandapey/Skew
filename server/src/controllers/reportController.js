...
export const dashboardStats = asyncHandler(async (req, res) => {
  ...
  const [
    ...
    ClientProject.estimatedDocumentCount(),
  ...
  res.json({
    employees, projects, clients, pendingLeaves,
    trends: {
      employees: 0, projects: 0, clients: 0, pendingLeaves: 0,
    ...
    activities: feed, meetings: [], tasks: [],
...
// Average working/overtime hours by weekday.
const byWeekday = (arr) => {
  const m = {}
  arr.forEach((x) => {
    if (!x.date) return
    ...
    b.overtime += x.overtimeHours || 0
    b._n += 1
  })
  return DAYS.filter((d) => m[d]).map((d) => {
    const b = m[d]
    return { day: d, hours: +(b.hours / (b._n || 1)).toFixed(1), overtime: +(b.overtime / (b._n || 1)).toFixed(1) }
  })
}
...
const computeAttendance = async (query = {}) => {
  ...
  const totalOvertime = +(sum(rows, (r) => r.overtimeHours || 0)).toFixed(1)
  ...
  const table = rows.map((r) => ({
    ...
    workingHours: r.workingHours, overtimeHours: r.overtimeHours,
  }))
  return {
    kpis: {
