...
import { startLeaveScheduler, stopLeaveScheduler } from './services/leaveScheduler.js'
...
// --- Health check ---
app.get('/api/health', (req, res) => res.json({ status: 'ok', service: 'Skew Enterprise Hub API' }))

// --- Routes ---
app.use('/api/auth', authRoutes)
// Broadcast a `resource:changed` event to internal users on every write.
app.use('/api/employees', withEmit(employeeRoutes, 'employees'))
app.use('/api/hr', withEmit(hrRoutes, 'hr'))
app.use('/api/attendance', withEmit(attendanceRoutes, 'attendance'))
app.use('/api/leave', withEmit(leaveRoutes, 'leave'))
app.use('/api/project', withEmit(projectRoutes, 'projects'))
app.use('/api/finance', withEmit(financeRoutes, 'finance'))
app.use('/api/files', withEmit(fileRoutes, 'files'))
app.use('/api/reports', reportRoutes)
app.use('/api/dashboard', reportRoutes)
app.use('/api/notifications', withEmit(notificationRoutes, 'notifications'))
app.use('/api/calendar', withEmit(calendarRoutes, 'calendar'))
app.use('/api/announcements', withEmit(announcementRoutes, 'announcements'))
...
app.use('/api/users', withEmit(userRoutes, 'admin-users'))
...
connectDB(process.env.MONGO_URI).then(async () => {
  ...
  const shutdown = () => { stopLeaveScheduler(); gracefulShutdown(server) }
