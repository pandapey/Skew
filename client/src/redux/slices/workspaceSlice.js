...
// Default dashboard widget order. Widget ids must match `widgetRegistry.js`.
// Kept here (not in the registry) so a user's saved order survives new
// widgets being added later — new ids are appended by `workspaceService`.
// Quick Actions is intentionally NOT a registry widget: per the brief it's a
// persistent, always-visible productivity bar, not something to hide/reorder.
...
// Storage layer for per-user personalization (widget layout, notes,
// bookmarks, favorites, pins, recent pages, focus mode, work timer log).
// This slice is an implementation detail — components should never import it
// directly; they go through `useWorkspace()` in `services/workspace/workspaceService.js`
// so the storage backend (today: redux-persist/localStorage) can later be
// swapped for a server-synced store without touching any component.
const workspaceSlice = createSlice({
  ...
  reducers: {
    ...
    ensureWidgetIds: (state, action) => {
      // Appends any newly-registered widget ids a user hasn't seen yet, so
      // future Phase 2+ widgets show up without wiping a saved layout.
