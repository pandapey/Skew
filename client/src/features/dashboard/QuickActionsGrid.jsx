...
// Persistent, always-visible quick-actions bar — shared verbatim by Dashboard
// and My Work so there's one implementation of "the 8 things I do most,"
// not two. Deliberately outside the customizable widget registry: it's a
// core productivity affordance, not something to hide/reorder.
export function QuickActionsGrid() {
  ...
  const actions = [
    ...
    { label: 'Apply Leave', icon: FiUserCheck, onSelect: () => navigate('/leave') },
    { label: 'Open Calendar', icon: FiCalendar, onSelect: () => navigate('/calendar') },
    // Phase 6.2 (Task 6): was '/my-work' (deleted duplicate page).
    { label: 'My Tasks', icon: FiCheckSquare, onSelect: () => navigate('/my-tasks') },
    // Phase 6.2 (Task 7): 'Upload File' and 'Download Payslip' removed per spec.
    // downloadPayslip.js had no other consumer and is deleted with them.
  ]
