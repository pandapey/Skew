...
// Per-tab configuration: KPI cards, charts, table columns and export mapping.
const TAB_CONFIG = {
  employees: {
  ...
  attendance: {
    ...
    charts: (d) => [
      ...
      { title: 'Working Hours / Day', node: <BarsChart data={d.charts.hoursTrend} xKey="day" bars={[{ key: 'hours', color: '#2563EB' }, { key: 'overtime', color: '#06B6D4' }]} /> },
    ],
    columns: [
      { key: 'employee' }, { key: 'department' }, { key: 'date' },
      { key: 'status', render: (r) => <Badge tone={badge(r.status)}>{r.status}</Badge> },
      { key: 'checkIn' }, { key: 'checkOut' },
      { key: 'workingHours', align: 'right' }, { key: 'overtimeHours', align: 'right' },
    ],
    exportColumns: [
      ...
      { header: 'Working Hours', accessor: 'workingHours' }, { header: 'Overtime', accessor: 'overtimeHours' },
    ],
...
export default function Reports() {
  // Phase 5.7 (Task 7): persist active tab + date range + department filter.
  ...
  // Phase 5.5 (Task 9): the 'category' filter existed only for the Inventory
  // report, so every remaining tab filters by department.
  const filterOpts = [
