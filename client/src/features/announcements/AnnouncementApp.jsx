            ...
            <div className="space-y-4">
              {pinned.length > 0 && (
                <div>
                  ...
                  <div className="space-y-4">
                    {pinned.map((p) => (
                      <PostCard key={p.id} post={p} onLike={likeMut.mutate} onComment={(id, b) => commentMut.mutate({ id, body: b })} onEdit={openEdit} onDelete={handleDelete} onTogglePin={handleTogglePin} readOnly={isEmployee} />
                    ))}
                  </div>
                </div>
              )}
